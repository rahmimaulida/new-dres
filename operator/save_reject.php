...
if(isset($_POST['submit'])){
    ...
    $info = "Ticket ".$get." Success Added By ".$_SESSION['name']." At "now()"";
    $codeinfo = "Add";
    ...
    $insert = "INSERT INTO `tbl_prod_reject`(`no_ticket`, `material_name`,`material_description`, `qty`, `plant`, `sector`, `line`, `issue`, `amount`, `action`, `status`, `pic`, `insertedBy`, insertDate) SELECT ".$get.", `material_name`, `material_description`, `qty`, `plant`, `sector`, `line`, `issue`, `amount`, `action`, `status`, `pic`, `insertedBy`, now() FROM `tempreject_".$_SESSION['username']."`";
